...
def upload_page():
    ...
    with col1:
        st.markdown("<h1 style='color: #007bff;'>Upload or Use Oracle Data</h1>", unsafe_allow_html=True)

        # Add a radio button to choose between file upload and using Oracle Data from GitHub
        data_choice = st.radio("Please choose an option:", ("Upload Your Data", "Use the Entire Oracle Data Table"))

        if data_choice == "Upload Your Data":
            ...
            try:
                ...
                data_expanded_url = 'https://raw.githubusercontent.com/Romanos-Rizk/AUB-capstone/main/data/data_expanded_for_model.parquet'

                data = pd.read_parquet(data_url)
                data_expanded = pd.read_parquet(data_expanded_url)

                st.success("Oracle data loaded successfully!")

                # Save 'data' and 'data_expanded' to session state so they can be used in other pages
                st.session_state['data'] = data
                st.session_state['data_expanded'] = data_expanded

                st.write("Data loading complete! Here's a preview of the data:")
                st.write(data.head())
                ...
                if st.button("Proceed to Home Page", key="proceed_button_oracle"):
                    st.session_state["data_uploaded"] = True
                    st.session_state["current_page"] = "Home"

            except Exception as e:
                st.error(f"An error occurred while loading the Oracle data: {e}")
