...
def render_page():
    ...
    if st.session_state.get("logged_in") and st.session_state.get("data_uploaded"):
        ...
        # Custom CSS for the wide, squared buttons and removal of unnecessary padding
        st.markdown(
            """
            <style>
            .nav-container {
            ...
            /* Remove default padding/margin from the body */
            .block-container {
                padding-top: 0px;
                padding-bottom: 0px;
            }
        ...
        # Custom navigation bar with buttons that fill the entire column width
        st.markdown('<div class="nav-container">', unsafe_allow_html=True)
        col1, col2, col3, col4, col5 = st.columns([1, 1, 1, 1, 1])  # Equal width for each column
        ...
        with col2:
            if st.button("Data Overview", key="overview_button", use_container_width=True):
                st.session_state["current_page"] = "Data Overview"
                st.experimental_rerun()

        with col3:
            if st.button("EDA", key="eda_button", use_container_width=True):
                st.session_state["current_page"] = "EDA"
                st.experimental_rerun()

        with col4:
            if st.button("Model Configuration", key="config_button", use_container_width=True):
                st.session_state["current_page"] = "Model Configuration"
                st.experimental_rerun()

        with col5:
            if st.button("Model Insights", key="insights_button", use_container_width=True):
